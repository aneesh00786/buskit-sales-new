...
/// Returns a list of BarChartGroupData to be used in your custom chart.
List<BarChartGroupData> createBarGroups({
  ...
  return allCategory.asMap().entries.map((entry) {
    ...
    // Use matching performance if available
    final CategoryPerformancee perf = isMonthly
        ? defaultPerf
        : categoryPerformance.firstWhere(
            (p) => p.category == category.category,
            orElse: () {
              dev.log('No match found in categoryPerformance for ${category.category}');
              return defaultPerf;
            },
          );

    // Use matching monthly performance if available
    ...
    // Decide which values to use based on flags
    final num target = isMonthly ? monthPerf.actualTarget ?? 0.0 : perf.actualTarget ?? 0.0;
    final num projection = isMonthly ? monthPerf.actualProjection ?? 0.0 : perf.actualProjection ?? 0.0;
    final num actual = isMonthly ? monthPerf.actualSales ?? 0.0 : perf.actualSales ?? 0.0;
  ...
  Widget getBottomTitles(double value, TitleMeta meta,List<Category> allCategory) {
  ...
  Widget getBottomTitlesDummy(double value, TitleMeta meta, List<Category> allCategory,) {
    Widget text = Transform.rotate(
      angle: -1.34 / 4,
      child: MyRegularText(
        label: allCategory[value.toInt()].category ?? '',
    ...
    Widget buildLegend({required Color color, required String label,required BuildContext context}){
    return Row(
      children: [
        CircleAvatar(
