...
void showValueOrderDialog(
  ...
  final filteredOrders = (deliveryData.order?.totalOrders ?? [])
      .where((orderDetails) => status == 5
          ? (orderDetails.orderStatus == 5 || orderDetails.orderStatus == 14)
          : orderDetails.orderStatus == status)
      .toList();

  final double totalSum = filteredOrders.isNotEmpty
      ? filteredOrders.map((e) => (e.orderTotal ?? 0.0) as num).fold<double>(0.0, (a, b) => a + b.toDouble())
      : 0.0;
                                ...
                                child: ConstrainedBox(
                                  ...
                                  child: DataTable(
                                    ...
                                    rows: filteredOrders.asMap().entries.map((entry) {
                                      ...
                                      String formattedDate = item.orderCreatAt != null
                                      ...
                                      return DataRow(
                                        cells: [
                                          ...
                                          // Customer
                                          DataCell(Center(child: Text(item.businessName ?? 'N/A', textAlign: TextAlign.center, style: const TextStyle(fontFamily: 'Poppins_Regular', fontSize: 12, fontWeight: FontWeight.w600, color: Colors.black87)))),
                                          // Status
