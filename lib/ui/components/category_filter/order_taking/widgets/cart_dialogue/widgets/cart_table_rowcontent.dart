    ...
    return groupedItems.map((groupedItem) {
      final taxDiscountAmount = ((groupedItem.detail.tax ?? 0.0) *
          ...
          ((double.tryParse(groupedItem.detail.discount?.toString() ?? '0') ??
                  0.0) /
              100));
      print('taxDiscountAmount:$taxDiscountAmount');
      ...
      final double quantity =
          (groupedItem.isPack == true || groupedItem.detail.packtype == 'Pack')
              ? (groupedItem.detail.pieces?.toDouble() ?? 1.0) *
                  (groupedItem.detail.count.toDouble())
              : groupedItem.detail.count.toDouble();
      print('qauntity form my carttt:$quantity');
      ...
      // This corresponds to the amount in your DataCell
      double baseSellAmount = sellPrice * qtyFactor;

      print('Base Sell Amount: $baseSellAmount');
...
double productQuantity = groupedItem.detail.count.toDouble();
      ...
      // 2. Get Discount Percentages
      double CustomerDiscount = (groupedItem.CustomerDiscount != null && groupedItem.CustomerDiscount! > 0)
          ? groupedItem.CustomerDiscount!
          : 0.0;

      num tieredDiscount = (groupedItem.tieredDiscount != null && groupedItem.tieredDiscount! > 0)
      ...
      // REVERTED TO OLD LOGIC: (Base Price * Quantity) * Percentage
      // This ensures the discount scales correctly when you increase quantity
      ...
      print('Calculated total discount amount: $totalDiscountAmount');
      ...
      print('price after disocunt in the cart_table ro:$priceAfterDiscount');

  double tax;
      if (groupedItem.taxAmount != null && groupedItem.taxAmount! > 0) {
        tax = groupedItem.taxAmount!;
        print('tax from backend:$tax');
      } 
      else {
      ...
      // groupedItem.taxAmount = tax; // Update the model with the new tax
      
      // 6. Final Price Logic
      double finalPrice;
      if (groupedItem.detail.inclTax == "incl_tax") {
        print('comes to inclusive tax section');
        finalPrice = priceAfterDiscount; 
      } else {
        print('comes to exclusive tax section');
    ...
    //   // 3. Calculate Total Discount Amount (Base Price * Quantity * %)
    //   // FIX: Added (* productQuantity) so discount scales with the button taps
    //   // double totalDiscountAmount = (baseSellAmount * productQuantity) * (totalDiscountPercent / 100.0);
    // double   totalDiscountAmount = baseSellAmount * (totalDiscountPercent / 100.0);
    ...
    //   // 5. Calculate Tax (Force recalculation)
    //   // FIX: Removed the "if (groupedItem.taxAmount != null)" check. 
    //   // We must recalculate this every time because quantity changes on button tap.
    //   double tax = priceAfterDiscount * (taxPercentage / 100);
    //   groupedItem.taxAmount = tax; // Update the model with the new tax
    ...
    //   groupedItem.finalPrice = finalPrice;
      ...
      // if (groupedItem.CustomerDiscount != null &&
      ...
      // num tieredDiscount;
      // if (groupedItem.tieredDiscount != null &&
      ...
      // totalDiscountAmount = baseSellAmount * (totalDiscountPercent / 100.0);

      // groupedItem.totalDiscountAmount = totalDiscountAmount;
      // print('Calculated discount amount (per product): $totalDiscountAmount');
      ...
      // if (groupedItem.detail.inclTax == "incl_tax") {
      //   finalPrice = priceAfterDiscount;
      //   print('included tax final price: $finalPrice');
      // } else {
      //   finalPrice = priceAfterDiscount + tax;
      //   print('excluded tax final price: $finalPrice');
      // }
      ...
      //   tax = groupedItem.taxAmount!;
      // } else {
      //   // Option B: Calculate tax using the Base Sell Amount
      //   tax = (baseSellAmount - totalDiscountAmount) * (taxPercentage / 100);
      //   groupedItem.taxAmount = tax;
      // }
      ...
      // if (groupedItem.detail.inclTax == "incl_tax") {
      //   finalPrice = (baseSellAmount * productquantity) - totalDiscountAmount;

      //   print('inclsuded tax final price:$finalPrice');
      // } else {
      //   finalPrice =
      //       ((baseSellAmount * productquantity) - totalDiscountAmount) + tax;

      //   print('excluded tax final price:$finalPrice');
      // }

      // groupedItem.finalPrice = finalPrice;
print('variation name of bulk item:${groupedItem.detail.variationName}');
      return DataRow(
        cells: [
          DataCell(
            SizedBox(
...
//       double totalDiscountAmount;
//       if (groupedItem.totalDiscountAmount != null &&
//           groupedItem.totalDiscountAmount! > 0) {
...
//       // ✅ LOGIC: Check if tax is already from backend, otherwise calculate it
//       if (groupedItem.taxAmount != null && groupedItem.taxAmount! > 0) {
//         // Option A: Use backend value
//         tax = groupedItem.taxAmount!;
//         print('Using backend tax amounttttttttttt: $tax');
//       } else {
//         // Option B: Calculate tax
//         tax = finalPrice * (taxPercentage / 100);
        
//         // Save calculated value to the item
//         groupedItem.taxAmount = tax; 
//         print('Calculated tax amount: $tax');
//       }
...
//                                     label: "Promo Offer",
//                                     percent: tieredDiscount,
//                                     amount: promoDiscountAmountPerUnit *
//                                         totalQuantity,
//                                     color: Colors.green.shade700,
//                                   ),
//                                   const SizedBox(height: 12),
