...
Widget build(BuildContext context) {
  ...
  final List<OrderData> displayList = widget.isSearchMode
      ? (widget.overrideOrders ?? [])
      : widget.orderController.orderDataList;

  final bool isEmpty = displayList.isEmpty;
  final bool isLoading = widget.isSearchMode
      ? widget.orderController.isSearchLoading.value
      : widget.orderController.isOrderLoading.value;

  int tabIndex = widget.hasOfflineOrders
      ? widget.selectedTabIndex - 1
      : widget.selectedTabIndex;

  // Show loading only if not search + normal loading, or if search + search loading
  if (isLoading) {
  ...
  if (isEmpty) {
    ...
    if (!widget.orderController.hasOfflineOrders.value && !_isOnline) {
      return const Center(
            ...
            children: [
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    OrderData orderData = displayList[index];
                    final bool hasCart = orderData.cart != null && orderData.cart!.isNotEmpty;
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            flex: 8,
                            child: hasCart
                                ? customerDetailsWidget(orderData.customer!.first)
                                : const SizedBox(),
                          ),
                        ],
                      ),
              ...
              if (!widget.isSearchMode)
                Container(
                  ...
                  child: Row(
                    children: [
                      OrderPaginationWidget(orderController: widget.orderController),
                      const Spacer(),
                    ],
                  ),
                ),
          ...
          child: SingleChildScrollView(
            ...
            child: SizedBox(
              ...
              child: OrdersBottomTitleRow(
                ...
                tabIndex: tabIndex,
                orderList: displayList, // Pass correct list
              ),
            ),
          ),
        ),
