...
class ProductReturnController extends GetxController {
  ...
  Future<void> fetchProductReturnDetails() async {
    ...
    try {
      ...
      isLoading(true);
      final response =
          await ApiWorker().getProductReturnDetails(orderId: orderId.value);
  ...
  bool _validateAllRows() {
    for (final cart in cartItems) {
      ...
      if (currentTotal > availableQty) {
        print(
            'Validation failed for ${cart.productName} (${cart.variationName}): '
      ...
      if (response['status'] == true) {
        ...
        Future.delayed(const Duration(seconds: 1), () {
          if (Get.isOverlaysOpen || Get.isDialogOpen == true) {
            Get.back(
                closeOverlays: true); // Close any open dialog/snackbar first
          }
